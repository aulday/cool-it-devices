...
public class BlueTermADS extends Activity {
...
//    	preamble[0]='W';
//    	mSerialService.write( preamble );
    	// Send 'D' to get sensor data
    	out[0]='D';
...
class TerminalEmulator {
    ...
    public void append(byte[] buffer, int base, int length) {
        for (int i = 0; i < length; i++) {
            ...
            try {
                if (BlueTermADS.LOG_CHARACTERS_FLAG) {
                    char printableB = (char) b;
                    if (b < 32 || b > 126) {
                        printableB = ' ';
                    }
                    Log.w(BlueTermADS.LOG_TAG, "'" + Character.toString(printableB)
                            + "' (" + Integer.toString(b) + ")");
                }
                process(b);
                mProcessedCharCount++;
            } catch (Exception e) {
                Log.e(BlueTermADS.LOG_TAG, "Exception while processing character "
    ...
    private void selectGraphicRendition() {
        for (int i = 0; i <= mArgIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                if (BlueTermADS.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
                    Log.w(BlueTermADS.LOG_TAG, String.format("SGR unknown code %d", code));
                }
            }
        }
    }
    ...
    private void logError(String error) {
        if (BlueTermADS.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
            Log.e(BlueTermADS.LOG_TAG, error);
        }
        finishSequence();
    }
...
*/
     ...
     */
    public void initialize(BlueTermADS blueTerm) {
    	mBlueTerm = blueTerm;
        mTextSize = 15;
        mForeground = BlueTermADS.WHITE;
        mBackground = BlueTermADS.BLACK;
        updateText();
